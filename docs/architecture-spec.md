...
A window closes after 256 edges or 65,536 cycles, whichever comes first; the features are snapshotted and the counters restart. Inference is a serial multiply-accumulate: one ternary weight per cycle into a single 16-bit accumulator (add, subtract or skip), ReLU and a host-set right shift to 8 bits as each hidden unit completes, then an 8-way output layer. 72 x 8 + 8 x 8 = 640 weights at 2 bits plus 8 signed 8-bit hidden-unit thresholds = 1,344 bits, so a full pass takes 640 cycles plus 8 for the argmax, 648 cycles or 13 us at 50 MHz (the v1.0 text said one feature per cycle with 8 accumulators, which contradicts its own ~700-cycle figure and would need 8 adders). Class = argmax; confident = (best - second best) above a host-set margin; OOD = best logit below a host-set floor, plus an activity gate. OOD is the weakest link and is validated empirically (section 9), not assumed.
...
2. Train: quantisation-aware training with ternary weights and integer thresholds (numpy, `python -m shruti train`); every feature is 6 bits, and the pins are presented in a canonical order (sorted by activity) so the model does not depend on which physical pins the bus is wired to. The same pipeline bootstraps the first model from a protocol simulator that emits edge streams with jitter and glitches, before any hardware exists.
...
4. Load: the host writes 1,344 bits over SPI at boot, in under a millisecond.
...
Placed and routed, 24 Sep 2026 (LibreLane in the GDS action, commit 1d5c2fd): 16,940 cells after buffering and clock tree, 255,000 um2, 28% utilisation, 0 DRC and 0 antenna violations; power about 6.8 mW. Timing at 50 MHz closes at all three corners: setup slack +0.06 ns at slow (1.08 V, 125 C), +7.4 ns at typical and +11.2 ns at fast; no hold violations. The flow reports 16 max-slew warnings at the slow corner. Getting there took three runs. The first missed the slow corner by 5.0 ns and the second by 2.3 ns. Three structural changes closed it, none visible at the ISA level: a registered instruction fetch, a T - counter register, and a registered filter output. The slow-corner margin is thin, and the Ear and Watch will add load on the same input path, so every block that lands from here gets a place-and-route run before the next starts.
...
Method: symbolic execution of the ISS. A program is at most 32 words, its loops are bounded by the X and Y counters, and its timing is arithmetic on T, so with the data bytes and B left symbolic the executor produces the pin trace as symbolic edge times and Z3 decides the contract (linear integer arithmetic for timing, bit-vectors for data). The UART transmit contract over all 256 bytes and every B from 8 to 65,535 cycles closes in about 0.5 s (prover v0, two symbolic frames with symbolic arrival times), and broken variants return counterexamples that the concrete ISS reproduces. An induction proof over the loop head then extends it to any number of frames: the base case and the step together take about 0.2 s. Reactive contracts followed: the UART receiver is proved against a symbolic peer with edge jitter up to P/16 (P = 16..65,535, and from P = 9 without jitter margin), and the SPI master's timing and data in both directions against a slave answering 1 or 2 cycles after each edge (P = 5..65,535). The I2C master is proved for one write transaction against a symbolic slave that ACKs or NACKs each byte and stretches the clock (P = 65..2,000, about 25 s); the bus-free time between transactions is measured on the ISS but not yet proved. Reactive programs (slave, receive) are checked against a symbolic input trace constrained by the peer's contract.
